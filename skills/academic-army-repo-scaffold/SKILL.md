...
and documentation-only notes, stop and report that scaffold generation failed. A
...
- Empty dependency declarations are allowed only when README and REFERENCES
  explicitly say no installable dependencies are declared for the relevant
  dependency bucket and explain why no dependencies were declared.
...
- A fixed top-level test directory is a failure unless the selected template or
  ...
  ecosystem convention before creating any test layout note.
- Do not create detailed test subdirectories from guessed coding-plan test
  capabilities. Test structure should follow the template or target ecosystem
  until concrete code defines reliable test boundaries.
- `REFERENCES.md` must be category-driven, not narrative-only. It must include
  ...
  comparison, implementation-reference, and rejected-candidate classifications.
...
- README, REFERENCES, and harness explanation files must use present-state,
  objective language. Do not use terms such as `future`, `placeholder`,
  `scaffold stage`, `will be implemented later`, or `to be filled` in generated
  user-facing docs.
...
- generated or conventional test structure, and how the repository should
  describe that structure without fighting the ecosystem
...
- **Runtime installable dependencies:** packaging is clear, license is
  ...
  the source is suitable to declare for direct use by runtime code.
...
`REFERENCES.md` or the README contract, unless they have a clear artifact
ownership role.

Testing is template-informed, not a fixed overlay. First inspect the selected
initializer's generated test structure. If it exists, preserve that structure
without adding guessed functional subdirectories. If the initializer does not
generate tests, use DeepResearch evidence for the target ecosystem's conventional
test layout and create only the smallest compatible test entry or layout note
inside that ecosystem convention. Do not create a fixed top-level test directory
unless the template or DeepResearch-supported ecosystem convention justifies it.
...
- how the repository's test layout follows the selected template or ecosystem
- if no tests were generated, which DeepResearch sources justify the chosen
  minimal test entry or layout note

Create additional source/package directories conservatively. The
template-generated source layout is the source of truth. Add source namespace
reservations only when they are directly implied by the coding plan's logical
modules or by the selected template's conventions, and document their current
responsibility rather than treating them as finalized architecture. Avoid filling
the template-selected source layout with many empty directories merely to mirror
every planning noun. If the relationship between top-level `harness/` and
executable harness code is represented in source reservations, use an
...
- generated or DeepResearch-supported test layout, and how the README explains
  its source and responsibility
...
- selected installable dependencies, or the explicit decision that no
  installable dependencies are declared for a dependency bucket
...
- test layout source and any concise test-layout note
...
documentation, test-layout provenance, fixed overlay directories, harness
coverage, template-aligned test layout, and objective documentation boundaries.
...
Harness explanation files must use objective, present-state language. Avoid
stage words such as `future`, `placeholder`, `scaffold stage`,
`will be implemented later`, or `to be filled`.

Do not derive detailed test directories from coding-plan test capabilities in
this initialization stage. Concrete modules, boundaries, fixtures, and test
granularity are not reliable before code exists. Preserve the generated
template's test structure when it exists. If no test structure was generated,
create only a minimal test entry or a concise test-layout note supported by
DeepResearch evidence for the selected ecosystem.

The test note, if created, should say that the test layout follows the selected
template or target ecosystem and that tests serve code functional correctness.
Do not pre-create guessed directories for data loading, metrics, export, CLI, or
other specific test categories.

Harness/test notes must not contain working harness logic, test code, fixture
data, or claims that validation is already implemented.

Keep top-level `harness/` semantically separate from source code. Top-level
`harness/` is for paper-goal harness descriptions and experiment workflow
ownership. Any package/module path associated with executable harness code must
be named as implementation code, not as a second copy of the harness
documentation tree.
...
- classify it as a runtime installable dependency, development/test
  installable dependency, reference-only source, rejected candidate, or
...
- declare runtime dependencies in the template's native runtime dependency
  mechanism only when they are appropriate for direct import/use by project code
...
- state that no runtime installable dependencies are declared in README,
  README.zh-CN, REFERENCES, and REFERENCES.zh-CN; Chinese docs may translate
...
Do not use concrete test-framework language in README, test layout notes, or
...
- harness locations and experiment-harness responsibilities
- generated or researched test structure and its source
- distinction between top-level `harness/` for paper evaluation specifications
  and the template-selected source layout
- why any additional non-fixed overlay directories were created beyond the
  template layer and fixed experiment overlay

README must use present-state, functional language. It should describe what the
repository contains, how to install dependencies, and what each directory does.
Do not use `future`, `placeholder`, `scaffold stage`,
`will be implemented later`, or `to be filled` in README files.
Do not make unfinished code the theme of the README.
...
- state which installable dependency buckets are declared, separating runtime
  dependencies from development/test dependencies when either bucket exists
...
harness list, test layout source, dependency decision, installation strategy,
and references.
...
- `reuse_decision`: generated-from, retained, adjusted, installable-dependency,
  consulted, cited-only, rejected, reference-only, or implementation reference
...
- whether it generated a test structure; if not, which DeepResearch-supported
  ecosystem convention is used for the test layout
...
- intended module, harness, test role, or scaffold purpose
...
reference-only or implementation reference. Do not copy or port implementation
code during this initialization task.
...
template files, template comparison, rejected candidates, and implementation
references.
...
3. Extract scaffold requirements: ecosystem signals, expected implementation
   direction, experiment types, harness categories, test layout needs, input data
...
11. Classify external libraries and tools as runtime dependencies,
   development/test dependencies, reference-only sources, or implementation
   references. Write selected dependencies into the template's
...
14. Add only plan-derived extra overlay directories that have clear
   artifact ownership, and document why they exist.
15. Add semantic harness folders under `harness/` with explanation
   files.
16. Preserve the generated test structure, or add only a minimal
   DeepResearch-backed test entry or test-layout note when the template provides
   no test structure. Do not create guessed fine-grained test directories.
...
- the test structure follows the generated or DeepResearch-supported ecosystem
  layout
- the scaffold did not create guessed fine-grained test directories before code
  exists
- README describes the test layout source and the boundary between harness and
  tests
...
- runtime and development/test dependencies are declared in the target
  ecosystem's dependency configuration, or an explicit no-dependencies-declared
  decision is recorded for each empty bucket
...
- README, REFERENCES, and harness explanation files use present-state objective
  language and do not include stage terms such as `future`, `placeholder`,
  `scaffold stage`, `will be implemented later`, or `to be filled`
- generated docs do not describe unimplemented functionality as working
- source reservations are either template-generated or explicitly documented as
  minimal manual reservations derived from the coding plan
...
- installation sections only defer setup until dependencies are added, or
...
- selected development/test tools appear in docs or test layout notes but not in the
...
- the scaffold creates guessed fine-grained test directories from coding-plan
  test categories before concrete code defines reliable test boundaries
...
- README, REFERENCES, or harness explanation files use stage terms such as
  `future`, `placeholder`, `scaffold stage`, `will be implemented later`, or
  `to be filled`
...
- installation commands were executed, dependencies were resolved/downloaded for
  ...
  resolution during this initialization task
- redundant documentation-only manifest or local references directory is created
  without user request, target-template need, or clear artifact ownership
...
- harness folders reserved under `harness/`
- test layout notes kept in the template-generated or DeepResearch-backed
