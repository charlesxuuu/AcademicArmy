...
初始代码仓库应为harness和test分别预留两个顶层文件夹。
每一种harness都应在harness文件夹下有独立子文件夹，用于放置该harness相关的入口、配置、说明、样例数据、结果schema或其他必要内容。
每一种test都应在test文件夹下有独立子文件夹，用于放置该类测试相关的测试脚本、fixtures、mock data或说明内容。
...
初始代码仓库应包含项目说明文档，简要说明仓库用途、核心模块、harness目录、test目录、结果artifact约定和后续实现入口。
README或developer notes应说明哪些部分是已搭好的框架，哪些部分是后续写代码skill需要补全的实现点。
如果仓库复制或改写了外部代码片段，应在合适的文档或源文件注释中保留来源、license和必要attribution。
文档中解释结构时，应优先用模块名、harness名、test名、method名或自然简称指代内容，而不是依赖抽象编号互相引用。

初始仓库创建前，skill应形成简短的内部仓库结构决策，明确语言和框架选择、核心目录、harness目录、test目录、配置机制、接口抽象、artifact schema和静态检查方式。
...
静态检查完成后，skill应确认仓库结构、关键文件、harness目录、test目录、依赖声明、配置入口、模块接口和结果artifact约定都已经创建。
...
**总原则**：这个skill负责把论文蓝图、experiment plan和coding plan落成一个静态、规整、可扩展的初始代码仓库；它应在用户指定路径内创建真实文件结构，为harness、test、method实现、结果导出和后续代码实现预留清晰抽象，同时通过deepresearch现场选择合适语言、框架、工具和最佳实践。
